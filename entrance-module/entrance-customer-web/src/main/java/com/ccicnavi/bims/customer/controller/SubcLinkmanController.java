...
public class SubcLinkmanController {
    ...
    **/
    ...
    private ResultT listSubcLinkman(@RequestBody SubcontractorDTO subcontractorDTO){
        log.debug("开始获取联系人列表根据分包商uuid Param:" + JSON.toJSONString(subcontractorDTO)  + " Time: " + new Date());
        //判断参数是否存在，如果不存在直接返回
        ...
        try {
            ...
            if(subLinkmanDOList != null && subLinkmanDOList.size() > 0){
                log.debug("获取联系人列表结果：" + JSON.toJSONString(subcontractorDTO));
                return ResultT.success(subLinkmanDOList);
            }
            log.debug("根据分包商获取联系人列表异常");
    ...
    private ResultT insertSubcLinkman(@RequestBody SubLinkmanDO subLinkmanDO){
        log.debug("开始保存分包方联系人信息 Param:" + JSON.toJSONString(subLinkmanDO)  + " Time: " + new Date());
        try {
            Integer count = subcLinkmanService.insertSubcLinkman(subLinkmanDO);
            if(count >= 1){
                log.debug("保存分包方联系人信息结果：保存数量" + count);
                return ResultT.success();
            }
            log.debug("保存分包方联系人信息异常");
    ...
    private ResultT updateSubcLinkman(@RequestBody SubLinkmanDO subLinkmanDO){
        log.debug("开始更新分包方联系人信息 Param:" + JSON.toJSONString(subLinkmanDO)  + " Time: " + new Date());
        ...
        try {
            ...
            if(count >= 1){
                log.debug("更新分包方联系人信息结果：更新数量" + count);
                return ResultT.success();
            }
            log.debug("更新分包方联系人信息异常");
    ...
    private ResultT deleteSubcLinkman(@RequestBody SubLinkmanDTO subLinkmanDTO){
        log.debug("开始移除分包方联系人信息 Param:" + JSON.toJSONString(subLinkmanDTO)  + " Time: " + new Date());
        ...
        try {
            ...
            if(count >= 1){
                log.debug("移除分包方联系人信息结果：移除数量" + count);
                return ResultT.success();
            }
            log.debug("移除分包方联系人信息异常");
    ...
    private ResultT getSubcLinkman(@RequestBody SubLinkmanDO subLinkmanDO){
        log.debug("开始获取分包方联系人信息 Param:" + JSON.toJSONString(subLinkmanDO)  + " Time: " + new Date());
        if(StringUtils.isEmpty(subLinkmanDO.getLinkmanUuid())){
            return ResultT.failure(ResultCode.PARAM_IS_BLANK);
        }
        try {
            ...
            if(subLinkman != null && !StringUtils.isEmpty(subLinkman.getSubcUuid())){
                log.debug("获取分包方联系人信息结果：" + JSON.toJSONString(subLinkman));
                return ResultT.success(subLinkman);
            }
            log.debug("获取分包方联系人信息异常");
